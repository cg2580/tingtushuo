...
from tingtushuo import create_app
from tingtushuo.extensions import db
from tingtushuo.models import User, Role, Photo, Comment, Tag
...
class BaseTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        admin_user = User(email='admin@tingtushuo.com', name='Admin', username='admin', confirmed=True)
        admin_user.set_password('123')
        normal_user = User(email='normal@tingtushuo.com', name='Normal User', username='normal', confirmed=True)
        normal_user.set_password('123')
        unconfirmed_user = User(email='unconfirmed@tingtushuo.com', name='Unconfirmed', username='unconfirmed',
                                confirmed=False)
        unconfirmed_user.set_password('123')
        locked_user = User(email='locked@tingtushuo.com', name='Locked User', username='locked',
                           confirmed=True, locked=True)
        locked_user.set_password('123')
        locked_user.lock()

        blocked_user = User(email='blocked@tingtushuo.com', name='Blocked User', username='blocked',
